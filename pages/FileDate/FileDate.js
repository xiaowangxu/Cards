...
Page({
    ...
    onLoad: function (options) {
        ...
        this.staticData.date = date
        let uiddata = this.file_Cards_with_UID(tables, this.get_MainAndCourseCollectionID())
    ...
    onPullDownRefresh: function () {
        let uiddata = this.file_Cards_with_UID(app.globalData.tables, this.get_MainAndCourseCollectionID())
        this.staticData.cards = uiddata
        this.setData({
            cards: uiddata
        })
    ...
    file_Cards_with_UID: function (tables, list = [{
        collectionid: 'Main',
        path: '主页',
        name: '主页'
    }]) {
        ...
        for (let t = 0; t < list.length; t++) {
            ...
            while (subtable.length > 0) {
                ...
                for (let i = 0; i < table.length; i++) {
                    let item = table[i]
                    if ((item.type === 'Todo' || item.type === 'Passage' || item.type === 'Picture') && util.matchCard_Date(item, this.staticData.date, this.staticData.date)) {
                        if (!samelayer) {
